...
class ContactService(ContactServiceInterface, BaseService):
    ...
    async def _get_instructions(self, agent):
        ...
        self.log.debug(f"代理 {agent.paw} 请求指令，找到 {len(ops)} 个操作")
        ...
        for op in ops:
            ...
            if hasattr(op, 'agent_ids') and op.agent_ids:
                ...
                access_reason = f"代理ID匹配: {agent.paw} in {op.agent_ids}"
            elif op.group and op.group != 'custom':
                # 如果操作指定了组，检查代理是否属于该组
                can_access = op.group == agent.group
                access_reason = f"组匹配: {agent.paw} 属于组 {op.group}"
            else:
                # 如果没有指定组或代理ID，所有代理都可以访问
                can_access = True
                access_reason = "无限制访问"
            
            self.log.debug(f"操作 {op.name}: {access_reason} -> {can_access}")
            
            if can_access:
                accessible_ops.append(op)
        
        self.log.debug(f"代理 {agent.paw} 可以访问 {len(accessible_ops)} 个操作")
        
        # 只从可访问的操作中获取指令
        for link in [c for op in accessible_ops for c in op.chain
        ...
        self.log.debug(f"代理 {agent.paw} 获得 {len(instructions)} 条指令")
        return instructions
    ...
    async def _add_agent_to_operation(self, agent):
        ...
        for op in await self.get_service('data_svc').locate('operations', match=dict(finish=None)):
            # 检查代理是否应该被添加到操作中
            should_add_agent = False
            
            if hasattr(op, 'agent_ids') and op.agent_ids:
                # 如果操作指定了具体的代理ID列表，检查当前代理是否在列表中
                should_add_agent = agent.paw in op.agent_ids
            elif op.group:
                # 如果操作指定了组，检查代理是否属于该组
                should_add_agent = op.group == agent.group
            else:
                # 如果没有指定组或代理ID，添加所有代理
                should_add_agent = True
