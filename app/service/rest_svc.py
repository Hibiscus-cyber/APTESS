...
class RestService(RestServiceInterface, BaseService):
    ...
    async def construct_agents_for_group(self, group, agent_ids=None):
        if agent_ids and len(agent_ids) > 0:
            # 如果提供了具体的代理ID列表，则根据ID选择代理（优先级最高）
            all_agents = await self.get_service('data_svc').locate('agents')
            selected_agents = [agent for agent in all_agents if agent.paw in agent_ids]
            self.log.debug(f"根据代理ID列表选择了 {len(selected_agents)} 个代理: {agent_ids}")
            return selected_agents
        elif group and group != 'custom':
            # 如果提供了组名且不是custom，则根据组选择代理
            group_agents = await self.get_service('data_svc').locate('agents', match=dict(group=group))
            self.log.debug(f"根据组 '{group}' 选择了 {len(group_agents)} 个代理")
            return group_agents
        else:
            # 如果没有指定组或代理ID，返回所有代理
            all_agents = await self.get_service('data_svc').locate('agents')
            self.log.debug(f"选择了所有 {len(all_agents)} 个代理")
            return all_agents
    ...
    async def _build_operation_object(self, access, data):
        ...
        agents = await self.construct_agents_for_group(group, agent_ids)
        ...
        return Operation(name=name, planner=planner[0], agents=agents, adversary=adversary,
                         group=group, agent_ids=agent_ids, jitter=data.pop('jitter', '2/8'), source=next(iter(sources), None),
