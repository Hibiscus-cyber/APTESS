...
function validateAndSave() {
    ...
    if ((agentConfig.value.untrusted_timer !== 0 && !agentConfig.value.untrusted_timer) || agentConfig.value.untrusted_timer < 0) {
        validation.untrustedTimer = "不受信计时器必须大于等于0！";
    } else {
        validation.untrustedTimer = "";
    }
    // Implant name
    if (!agentConfig.value.implant_name) {
        validation.implantName = "木马名称不能为空！";
    } else {
        ...
        .modal-card-body 
            table(style="table-layout: fixed; width: 100%;")
                ...
                tbody
                    tr
                        ...
                        td
                            .is-flex.is-align-items-center 
                                label.mr-3(style="display:inline-block; width:200px") 最小值
                                ...
                                label.mr-3(style="display:inline-block; width:200px") 最大值
                    ...
                    tr
                        th 启动能力
                            img(src="../../assets/img/info-icon.png" alt="info-icon" style="width:7%; margin-left:7px;" title="若需精确搜索，请以引号开头输入关键字")
                        td
                            ...
                            .field.has-addons(v-if="isAddingBootstrapAbility")
                                .control.is-expanded.auto-complete()
                                    input.input(v-model="abilitySearchQuery" placeholder="搜索能力…" @keyup="abilitySearchResults = searchAbilitiesByName(abilitySearchQuery)")
                                ...
                                .control 
                                    a.button(@click="isAddingBootstrapAbility = false") 取消
                    tr
                        th 死亡触发能力
                            img(src="../../assets/img/info-icon.png" alt="info-icon" style="width:7%; margin-left:7px;" title="若需精确搜索，请以引号开头输入关键字")
                        td
                            ...
                            .field.has-addons(v-if="isAddingDeadmanAbility")
                                ...
                                .control 
                                    a.button(@click="isAddingDeadmanAbility = false") 取消
            p.help * 此为全局设置，将影响所有新建和现有代理。
        footer.modal-card-foot.is-flex.is-justify-content-flex-end 
            button.button(@click="modals.agents.showConfig = false") 关闭
