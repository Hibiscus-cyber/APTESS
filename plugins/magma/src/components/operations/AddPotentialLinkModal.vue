...
.modal(:class="{ 'is-active': props.active }")
    ...
    .modal-card
        ...
        .modal-card-body(v-if="!selectedPotentialLink.name")
                        ...
                        .control(v-if="canSelectAgent")
                            .select.is-fullwidth
                                select(v-model="filters.agent")
                                    option(disabled default value="") 选择一个代理
                        ...
                        .control
                            .select.is-fullwidth
                                select(v-model="filters.executor")
                                    option(disabled default value="") 选择一个执行器
            ...
            .field.is-horizontal
                ...
                .field-body
                    .field
                        .control.is-expanded
                            input.input(v-model="filters.searchQuery" type="text" placeholder="搜索链接…")
            .field.is-horizontal
                .field-label.is-normal 
                    label.label 策略（Tactic）
                .field-body
                    .field
                        .control
                            .select.is-fullwidth
                                select(v-model="filters.tactic")
                                    option(default value="") 所有策略
                                    option(v-for="tactic in abilityStore.tactics" :key="tactic" :value="tactic") {{ tactic }}
            .field.is-horizontal
                .field-label.is-normal 
                    label.label 技术（Technique）
                .field-body
                    .field
                        .control
                            .select.is-fullwidth
                                select(v-model="filters.technique")
                                    option(default value="") 所有技术
            ...
            .is-flex.is-justify-content-space-between.content
                h3 选择一个潜在链接
                h3.m-0 {{ `${filteredAbilities.length} 个可用链接` }}
        ...
        .modal-card-body(v-else)
            ...
            label.label.has-text-centered.mt-3 链接命令
            CodeEditor(v-model="potentialLinkCommand" language="bash" line-numbers)
            p.help 编辑事实模板 (<code>\#{...}</code>) 可能会导致意外行为。

            div(v-if="potentialLinkFields.length") 
                label.label.has-text-centered 事实模板
                p.mb-2 该链接需要事实（facts）才能正确运行。请为每个事实选择至少一个值，或手动输入自定义值。
                .block(v-for="factName in Object.keys(selectedPotentialLinkFacts)")
                    span.icon-text 
                        ...
                        span.icon.has-text-warning(
                            ...
                            v-tooltip="`当前存在未填写的事实模板。仍可添加此链接，但可能无法正确执行。`")
                            font-awesome-icon(icon="fa-exclamation-triangle")
                    .control
                        input.input(v-model="selectedPotentialLinkFacts[factName].customValue" type="text" placeholder="输入自定义值")
        ...
        footer.modal-card-foot
            ...
            button.button(v-if="selectedPotentialLink" @click="selectedPotentialLink = ''") 返回
            button.button.is-primary.ml-auto(@click="emit('select', potentialLinksToAdd)") 添加 {{ potentialLinksToAdd.length }} 个潜在链接{{ potentialLinksToAdd.length > 1 ? '' : '' }}
</template>
