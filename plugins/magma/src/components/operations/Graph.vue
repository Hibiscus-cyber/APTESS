  ...
  .graph-container(:style="{height: isGraphOpen ? '34rem' : '0px'}")
    .tooltip-wrapper()
      ...
      .tooltip(ref="tooltip" :style="{...tooltipPos, opacity: tooltipOpacity}")
        ...
        span(v-if="targetNodeId") 平台: {{ nodes[targetNodeId].platform }}
        div
          span(v-if="targetNodeId") {{nodes[targetNodeId].agents.length}} 个代理
          |
          span(v-if="targetNodeId && nodes[targetNodeId].agents.length != 1") 
    .sidebar(:style="{width: isSidebarOpen && isGraphOpen ? '500px' : '65px'}")
      ...
      table.table.sidebar-table(v-if="isSidebarOpen && isGraphOpen" :style="{opacity: !isSidebarOpen ? 0 : 1}")
        tbody(v-if="selectedNodeId")
          ...
          tr
            th 代理 ({{nodes[selectedNodeId].agents.length}})
            td
              ul#agent-list
      ...
      span(v-if="isSidebarOpen && isGraphOpen" :style="{opacity: !isSidebarOpen ? 0 : 1}") 最近动作
      table.table.sidebar-table(v-if="isSidebarOpen && isGraphOpen" :style="{opacity: !isSidebarOpen ? 0 : 1}")
        tbody(v-if="selectedNodeId")
          tr
            th 状态
