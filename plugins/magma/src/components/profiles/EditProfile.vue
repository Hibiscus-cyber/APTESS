<script setup>
import { reactive, ref, inject, computed,defineProps } from "vue";
...
// 监听父组件传入的 profile，对 validation 进行赋值
watch(
  () => props.selProfile,
  (val) => {
    if (!val) return;
    validation.profile_id = val.profile_id || "";
    validation.name = val.name || "";
    validation.file_name = val.file_name || "";
    validation.description = val.description || "";
    validation.file_type = val.file_type || "";
    validation.platform = Array.isArray(val.platform)
      ? val.platform
      : val.platform
      ? [val.platform]
      : [];
    validation.tactic = Array.isArray(val.tactic)
      ? val.tactic
      : val.tactic
      ? [val.tactic]
      : [];
    validation.risk = val.risk || "Medium";
    fileName.value = val.file_name || fileUploadPlaceholder;
    console.log("已载入 selProfile:", val);
  },
  { immediate: true }
);
...
function closeModal() {
  modals.value.profile.showEdit = false;
}

async function onSubmitClick($event) {
  $event.preventDefault();
  try {
    // 带 profile_id 的整个表单对象发给后端
    await profileStore.editProfile($api, validation);
...
<template lang="pug">
.modal(:class="{ 'is-active': modals.profile.showEdit }")
  .modal-background(@click="closeModal()")
  .modal-card
    header.modal-card-head.header-bar
      .header-left
        p.modal-card-title 编辑载荷画像
        p.header-subtitle 对已上传载荷画像进行编辑
      ...
      form(@submit.prevent)
        .form-grid
          ...
          .form-section
            ...
            .field
              label.label 字段
              .display-box {{validation.profile_id}}
              ...
              .control
                .checkbox-group.checkbox-group--scroll
                  label.checkbox-item(v-for="p in platformOptions" :key="p.value")
                    input(type="checkbox" :value="p.value" v-model="validation.platform")
              ...
              .control
                .checkbox-group.checkbox-grid.checkbox-group--scroll
                  label.checkbox-item(v-for="t in tacticOptions" :key="t.value")
                    input(type="checkbox" :value="t.value" v-model="validation.tactic")
