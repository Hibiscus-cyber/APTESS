...
.modal(:class="{ 'is-active': modals.schedules.showCreate }")
  ...
  .modal-card
    header.modal-card-head
      p.modal-card-title {{ scheduleStore.currentSchedule ? "更新" : "新建" }} 计划
    .modal-card-body
      .field.is-horizontal
        .field-label.is-normal
          label.label 计划表达式
        .field-body
          input.input(placeholder="* * * * *    <分钟> <小时> <日> <月> <星期>" v-model="schedule")
          label.label.ml-3.mt-1.has-text-danger {{ `${validation.schedule}` }}
      .field.is-horizontal
        .field-label.is-normal
          label.label 任务名称
        .field-body
          input.input(placeholder="行动名称" v-model="taskName" :disabled="scheduleStore.currentSchedule")
          ...
          .control
            .select
              select(v-model="selectedSource" :disabled="scheduleStore.currentSchedule")
                option(disabled selected value="") 选择知识源
                option(v-for="source in sources" :key="source.id" :value="source") {{ `${source.name}` }}
      .field.is-horizontal
        .field-label.is-normal
          label.label 分组
        .field-body
          button.button(:class="{ 'is-primary': selectedGroup === '' }" @click="selectedGroup = ''") 全部分组
      ...
      .field.is-horizontal
        ...
        .field-body.is-grouped
          ...
          label.label.ml-3.mt-1(for="keepopen") 始终保持打开
          ...
          label.label.ml-3.mt-1(for="autoclose") 自动结束行动
      .field.is-horizontal
        .field-label
          label.label 运行状态
        .field-body.is-grouped
    ...
    footer.modal-card-foot.is-justify-content-space-between
      ...
      button.button.is-danger(@click="modals.schedules.showDelete = true" v-if="scheduleStore.currentSchedule") 删除
      div
        button.button(@click="closeModal()") 取消
        button.button.is-primary(@click="callApi().then(() => {})")
          span(v-if="scheduleStore.currentSchedule") 更新
          span(v-else) 创建
</template>
