...
table.table.is-striped.is-fullwidth
    ...
    tbody
        tr(v-for="(fact, index) of selectedSource.facts")
            td 
                input.input(v-if="factIndexToEdit === index" v-model="fact.trait" placeholder="输入Fact特征")
                div.is-flex.is-flex-direction-row(v-else)
                  span {{ fact.trait || "---" }}
                  #fact-source-icon.tag.ml-2(v-if="fact.origin_type" :class="getFactOriginColor(fact.origin_type)" v-tooltip="`来源：${fact.origin_type.toLowerCase()}`") {{ getFactOriginShort(fact.origin_type) }}
            td 
                input.input(v-if="factIndexToEdit === index" v-model="fact.value" placeholder="输入Fact值")
                span(v-else) {{ fact.value || "---" }}
            td 
                input.input(v-if="factIndexToEdit === index" v-model="fact.score" placeholder="输入分数")
