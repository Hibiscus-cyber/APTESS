...
.content
    h2 能力
    p “能力”是对 ATT&CK 策略/技术 的具体实现，可在运行中的代理上执行。能力包含要运行的命令、可运行的平台/执行器（例如：Windows / PowerShell）、需要包含的载荷，以及在 APTESS 服务器上用于解析输出的模块引用。
hr
...
.columns
    .column.is-2.m-0
        ...
        form
            .field
                .control.has-icons-left
                    input.input.is-small(v-model="filters.searchQuery" type="text" placeholder="搜索能力…")
                    span.icon.is-left
        ...
        button.button.is-fullwidth.mt-4(@click="clearFilters()") 清除筛选
        p.mt-2.has-text-centered
            strong {{ filteredAbilities.length }}&nbsp;
            | / {{ abilities.length }} 个能力
