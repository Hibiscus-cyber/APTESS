...
.content
  h2 外传文件
  p 外传文件是在一次行动中由主机上的代理上传到服务器的文件，默认保存在服务器路径 {{ exfilDir }}。
    br
    span 如果某次行动包含外传文件，您可以在此处查看并下载这些文件。
hr

p(v-if="numFiles === 0") 外传目录（{{ exfilDir }}）为空。
.buttons(v-else)
  button.button.is-primary.is-small(@click="toggleAllFiles()")
    span(v-text="selectedFiles.length < numFiles ? '选择' : '取消选择'")
    | &nbsp;全部文件
      ...
      ul.tree
        li.agentName-child
          ...
          ul.tree
            ...
            li.agent-child(v-show="!Object.keys(files[agentName][operation]).length") （无文件）
</template>
