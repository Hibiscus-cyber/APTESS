            ...
            .control.mr-2
                .select
                    select.has-text-centered(v-model="operationStore.selectedOperationID" @change="selectOperation()")
                        option(disabled selected value="") 选择一个行动 
                        option(v-for="operation in operationStore.operations" :key="operation.id" :value="operation.id") {{ `${operation.name} - ${operation.chain.length} 条指令 | ${getHumanFriendlyTimeISO8601(operation.start)}` }}
    ...
    .columns.m-0.p-1
            ...
            .buttons
                ...
                button.button(v-if="operationStore.isOperationRunning()" @click="showPotentialLinkModal = true")
                    span.icon
                        font-awesome-icon.fa(icon="fas fa-plus")
                    span 潜在链接
                button.button(@click="modals.operations.showDetails = true" type="button") 行动详情
        ...
        .column.is-4.pt-0.pb-0
            ...
            .is-flex.is-justify-content-center.is-align-items-center.pb-2
                a.icon.is-medium.ml-3.mr-3(v-if="!isRerun()" @click="operationStore.updateOperation($api, 'state', 'cleanup')" v-tooltip="'停止'")
                    font-awesome-icon.fa-2x(icon="fas fa-stop")
                a.icon.is-medium.ml-3.mr-3(v-if="!isRerun() && operationStore.operations[operationStore.selectedOperationID].state === 'paused' || operationStore.operations[operationStore.selectedOperationID].state === 'run_one_link'" @click="operationStore.updateOperation($api, 'state', 'running')" v-tooltip="'继续'")
                    font-awesome-icon.fa-2x(icon="fas fa-play")
                a.icon.is-medium.ml-3.mr-3(v-else v-if="!isRerun()" @click="operationStore.updateOperation($api, 'state', 'paused')" v-tooltip="'暂停'")
                    font-awesome-icon.fa-2x(icon="fas fa-pause")
                a.icon.is-medium.ml-3.mr-3(v-if ="!isRerun()" @click="operationStore.updateOperation($api, 'state', 'run_one_link')" v-tooltip="'单步运行'")
                    font-awesome-icon.fa-2x(icon="fas fa-play")
                    span.mt-5 1
                a.icon.is-medium.ml-3.mr-3(v-if="isRerun()" @click="operationStore.rerunOperation($api)" v-tooltip="'重新运行行动'")
        ...
        .column.is-4.is-flex.is-justify-content-right.is-align-items-center.is-flex-wrap-wrap.pt-0.pb-0
            ...
            .control(v-if="operationStore.isOperationRunning()")
                ...
                label.label(for="switchManual") {{ operationStore.operations[operationStore.selectedOperationID].autonomous ? '自动模式' : '手动模式' }} 
            .control(v-else)
                input.switch(
                    :checked="operationStore.operations[operationStore.selectedOperationID].autonomous === 1" 
                    :disabled="true"
        ...
        tr(v-for="(link, idx) in filteredChain" :key="link.id" :id="`link-${idx}`")
            ...
            td
                .dropdown.is-hoverable(v-if="link.output === 'True'")
                    .dropdown-trigger
                        button.button(aria-haspopup="true" aria-controls="dropdown-menu" @click="handleViewOutput(link)") 查看输出
                    .dropdown-menu.command-popup(role="menu")
                        .dropdown-content
                            OutputPopup(:link="link")
                span(v-else) 无输出
            td(v-if="operationStore.currentOperation.state === 'running'")
                a.icon(@click="operationStore.rerunLink($api, link)" v-tooltip="'重新运行链接'")
